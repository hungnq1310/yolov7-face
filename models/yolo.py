...
class IDetectHead(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), nkpt=None, ch=(), inplace=True, dw_conv_kpt=False): # detection layer
        super(IDetectHead, self).__init__()
...
class IDetectBody(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), nkpt=None, ch=(), inplace=True, dw_conv_kpt=False): # detection layer
        super(IDetectBody, self).__init__()
...
class IKeypoint(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            x_kpt = x[i][..., 6:].clone().detach() # torch.Size([3, 3, 80, 80, 17])
    ...
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        for m in self.model[-5:]:
            if isinstance(m, Detect) or isinstance(m, IDetect) or isinstance(m, IKeypoint) or isinstance(m, IDetectHead) or isinstance(m, IDetectBody) or isinstance(m, IKeypointBody):
                ...
                ins = self.forward(torch.zeros(1, ch, s, s))
                m.stride = torch.tensor([s / x.shape[-2] for x in ins[m.__class__.__name__]])  # forward
                #m.stride = torch.tensor([s / x.shape[-2] for x in self.forward(torch.zeros(1, ch, s, s))])  # forward
